...
#include <stdlib.h>
/**
 * replace_string - Function to replace the string %s within format, with argument.
 *
 * Description: this function searches all occurrence of the string %s within the 'format' string
 *				and replace with the 'argument' string.
 *
 * @format: A string which contains the specifier %s to replace it.
 * @argument: The string that replace the %s.
 *
 * Return: A new string where all %s occurrences have been replaced by the value of 'argument'
*/
char *replace_string(char *format, char *argument)
{
	...
	{
		if (format[index_format] == '%' && format[index_format + 1] == 's')
		{
